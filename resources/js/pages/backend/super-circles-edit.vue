<template>
  <div>
    <div class="card">
      ...
      <div class="card-body">
        <div class="row">
          <input-app v-if="mode === 'edit'" v-model="superCircle.name" label="Super Circle Name" type="text" :disabled="!$can(EDIT_SUPER_CIRCLES)"/>
          <input-app
            v-if="mode === 'new'"
        ...
        <div class="d-flex justify-content-center mt-2">
          <button class="btn ing-btn mr-2" @click="submit" :disabled="disabled" v-if="(mode == 'edit' && $can(EDIT_SUPER_CIRCLES)) || (mode == 'new' && $can(CREATE_SUPER_CIRCLES)) ">
            <span>Submit</span>
...
export default {
  ...
  computed: {
    superCircle() {
      ...
      const superCircle = superCircles.filter(sc => sc.id === this.$route.params.id)[0];
  ...
  methods: {
    submit() {
      ...
      if (this.mode === "edit") {
        this.$store.dispatch("backend-mgmt/updateSuperCircle", this.superCircle).then(
          res => {
        ...
        this.$store.dispatch("backend-mgmt/addNewSuperCircle", this.newSuperCircle).then(
          res => {
            this.$router.push({ name: "backend.super-circles" });
          },
          err => {
